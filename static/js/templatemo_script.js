...
jQuery(function () {
  ...
  //main menu
  $("#templatemo_banner_menu ul").singlePageNav({
    offset: $("#templatemo_banner_menu").outerHeight(),
  });
  //banner slide
  ...
  $("#prev_testimonial").click(function () {
    $.current_testimonial.effect("fade", {}, 200, function () {
      $.current_testimonial_prev =
        $.current_testimonial.index() == 0
          ? $(".testimonial_text").last()
          : $.current_testimonial.prev();
  ...
  $("#next_testimonial").click(function () {
    $.current_testimonial.effect("fade", {}, 200, function () {
      $.current_testimonial_next =
        $.current_testimonial.index() == $(".testimonial_text").last().index()
          ? $(".testimonial_text").first()
  ...
  $(window).on("load resize", function () {
    ...
    if ($(window).width() > 550) {
      ...
      $(".event_box_wap").each(function () {
        ...
        $(this)
          .children(".event_box_caption")
          .css({ top: "-" + hide_paragraph_height + "px" });
      });
    } else {
      $(".event_box_wap").height(img_height);
      $(".event_box_wap").each(function () {
    ...
    function () {
      if ($(window).width() > 550) {
        ...
        paragraph_height = $(this)
          ...
          .eq(1)
          .outerHeight();
        hide_paragraph_height =
          header_height + admin_height + paragraph_height + 10;
        $(this)
          .children(".event_box_caption")
          .stop()
          .animate({ top: "-" + hide_paragraph_height + "px" });
      }
    },
    function () {
      if ($(window).width() > 550) {
  ...
  $(window).on("load resize", function () {
    ...
    $(".time_line_wap").each(function () {
      ...
      if ($(window).width() < 970) {
        ...
        positon_left =
          $("#templatemo_timeline .container-fluid").position().left + 100;
        //put on right
        $(this).css({
          left: 70,
        ...
        $.timeline_right_position_top =
          $.timeline_right_position_top + $(this).outerHeight() + 40;
        $(this).prepend(
          '<a href="#" class="right_timer"><span class="glyphicon glyphicon-time"></span></a>'
        );
        ...
        $.timeline_left_position_top =
          $.timeline_left_position_top + $(this).outerHeight() + 40;
        $(this).prepend(
          '<a href="#" class="left_timer"><span class="glyphicon glyphicon-time"></span></a>'
        );
        $(this)
          .children("a.left_timer")
          .css({ left: $.timeline_item_width - 50 });
      } else if ($.timeline_right_position_top < $.timeline_left_position_top) {
        $("#templatemo_timeline .container-fluid").css({
          position: "relative",
        });
        $.timeline_right_position_top =
          $.timeline_old_left_position_top + 40 < $.timeline_right_position_top
            ? $.timeline_right_position_top
            : $.timeline_right_position_top + 40;
        //put on right
        $(this).css({
        ...
        $.timeline_left_position_top =
          $.timeline_old_right_position_top + 40 < $.timeline_left_position_top
            ? $.timeline_left_position_top
            : $.timeline_left_position_top + 40;
        //put on left
        $(this).css({
          left: 0,
      ...
      if ($.timeline_left_position_top > $.timeline_right_position_top) {
        $("#templatemo_timeline .container-fluid").height(
          $.timeline_left_position_top - 40
        );
  ...
  jQuery.fn.anchorAnimate = function (settings) {
    ...
    return this.each(function () {
      ...
      $(caller).click(function (event) {
        ...
        var destination =
          $(elementClick).offset().top -
          $("#templatemo_banner_menu").outerHeight();
  ...
  $(document).scroll(function () {
    document_top = $(document).scrollTop();
    event_wapper_top =
      $("#templatemo_events").position().top -
      $("#templatemo_banner_menu").outerHeight();
    if (document_top < event_wapper_top) {
      event_animate_num = event_wapper_top - document_top;
...
function initialize() {
  ...
  map = new google.maps.Map(
    document.getElementById("templatemo_contact_map"),
    mapOptions
  );
